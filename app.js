...
function renderHistoryList(clients) {
  ...
  clients.forEach(client => {
    const item = document.createElement('div');
    item.className = 'client-item';
    const notesPreview = client.notes && client.notes.trim() ? ` • ${client.notes.slice(0, 30)}${client.notes.length > 30 ? '...' : ''}` : '';
    item.innerHTML = `
      <div class="client-name">${client.name}</div>
...
function saveExtraHours() {
  ...
  const extraEntry = {
    ...
    hours: hours, seconds: Math.round(hours * 3600),
    description: description || 'Hores extres', billable: true
  };
