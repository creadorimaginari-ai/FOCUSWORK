...
function renderClientCard(client) {
  ...
  // Text tasca urgent (netejat de prefixos automàtics)
  const urgentRaw  = (client.tasks && client.tasks.urgent) || '';
  const urgentText = urgentRaw.replace(/^[⚠️🔴🟡📅][^\n]*\n?/, '').trim();
  const hasUrgent  = urgentText.length > 0;

  return `
    <button class="client-card ${hasUrgent ? 'card-has-urgent' : ''}" data-client-id="${client.id}">
      <div class="client-card-inner">

        ${hasUrgent ? `
        <div class="client-urgent-col">
          <div class="client-urgent-dot">🔴</div>
          <div class="client-urgent-text">${urgentText.substring(0, 55)}${urgentText.length > 55 ? '…' : ''}</div>
        </div>
        ` : ''}
        ...
        <div class="client-card-main">
          <div class="client-card-header">
            ...
            ${isBlocked ? '<span class="client-card-blocked" title="Bloquejat">🔒</span>' : ''}
          </div>
          ...
          <div class="client-card-info">
            ...
            ` : ''}
            <span class="client-card-time">⏱️ ${formatTime(client.total || 0)}</span>
          </div>
        </div>
